package com.toly1994.tolyservice.provider.adapter;
...
public class ChatRvAdapter extends RecyclerView.Adapter<ChatRvAdapter.MyViewHolder> {
    ...
    public ChatRvAdapter(List<SMSBean> data) {
        mData = data;
    }
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        SMSBean msgBean = mData.get(position);

        switch (msgBean.getType()) {//对不同的viewType设置不同的数据
            case 1:
            case 2:
                holder.mItemTV.setText(msgBean.getBody());
                break;
            case 0:
